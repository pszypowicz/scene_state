...
2. Skip a member that is missing, `unavailable`, `unknown`, whose current
   state string differs from the desired one, or whose desired state is
   `off` or `closed`.
...
6. The step description lists the measurement of each attribute it renders a
   field for, and `none` when no field has one.
...
- A member that is missing, `unavailable`, `unknown`, whose current state
  string differs from the desired one, or whose desired state is `off` or
  `closed`, is skipped during measurement. It does not make the flow fail.
